...
### testing for draft 6 specifications  ###
...
### testing for draft 7 specifications  ###
tsdir = joinpath(unzipdir, "JSON-Schema-Test-Suite-master/tests/draft7")
...
@testset "JSON schema test suite (draft 7)" begin
    @testset "$tfn" for tfn in filter(n -> occursin(r"\.json$",n), readdir(tsdir))
        fn = joinpath(tsdir, tfn)
        schema = JSON.parsefile(fn)
        @testset "- $(subschema["description"])" for subschema in (schema)
