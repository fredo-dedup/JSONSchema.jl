...
function validate(schema::CompiledSchema, x::Schema; kwargs...)
    return invoke(validate, Tuple{CompiledSchema,Any}, schema, x; kwargs...)
end

Base.isvalid(schema::CompiledSchema, x) = validate(schema, x) === nothing
function Base.isvalid(schema::CompiledSchema, x::Schema)
    return validate(schema, x) === nothing
end
